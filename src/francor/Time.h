...
public:
  ...
  /**
   * @brief sets this Duration by rate (1/s)
   * 
   * @param rate 
   */
  void fromRate(const double rate) noexcept
  {
    if(rate)
    {
      double s = 1/rate;
      _duration = Duration_t(s);
    }
  }
 ...
 * @param[in] t time point as francor::Time
 * @return given ostream
 */
inline std::ostream& operator<<(std::ostream& os, const francor::Time& t)
{
 ...
 * @param[in] os ostream for output
 * @param[in] d duration as francor::Duration
 * @return given ostream
 */
inline std::ostream& operator<<(std::ostream& os, const francor::Duration& d)
{
